...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser: CommandParser) -> None:
        parser.add_argument("game_id", type=str, help="Game ID (UUID format) to export")
        parser.add_argument(
            "--output",
            type=str,
    ...
    def handle(self, *args: Any, **options: Any) -> None:
        ...
        # Create CSV headers
        headers = ["Person Index", "Decision", "Decision Text", "Created At"]
        ...
        try:
            with open(output_file, "w", newline="", encoding="utf-8") as csvfile:
                ...
                for person in people:
                    ...
                    for attr in sorted_attributes:
                        value = (
                            person.attributes.get(attr, False)
                            if person.attributes
                            else False
                        )
                        row.append(value)
            ...
            raise CommandError(f"Error writing to file {output_file}: {e}") from e
